...
namespace HuntControl.WebUI.Helpers
{
    public static class QRCodeHtmlHelper
    {
        ...
        /// <returns></returns>
        public static MvcHtmlString QRCode(this HtmlHelper htmlHelper, string data, int size = 150, int margin = 4, QRCodeErrorCorrectionLevel errorCorrectionLevel = QRCodeErrorCorrectionLevel.Low, object htmlAttributes = null)
        {
            if (data == null)
            ...
            var url = string.Format("http://chart.apis.google.com/chart?cht=qr&chld={2}|{3}&chs={0}x{0}&chl={1}&choe=UTF-8", size, HttpUtility.UrlEncode(data), errorCorrectionLevel.ToString()[0], margin);
